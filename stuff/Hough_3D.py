...
def make_image(line3, k_centers, point_index, make_gif, name):
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    line1 = ax.scatter3D(line3[:, 0], line3[:, 1], line3[:, 2], s=18,
                         c='r')  # colormap[point_index])#colormap[categories])
...
colormap = np.array(
    ['b', 'g', 'r', 'deeppink', 'y', 'c', 'm', 'gold', 'purple', 'pink', 'tan', 'lightgreen', 'sandybrown', 'olive'])
